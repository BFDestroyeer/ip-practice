...
def salt_and_pepper_noise(image, intensity):
    if intensity > 50:
        raise ValueError('Too high noise intensity')
...
def averaging_filter(image, radius):
    if radius < 1:
        raise ValueError('Wrong radius')
    ...
    for y in range(image.shape[0]):
        for x in range(image.shape[1]):
            ...
            green = 0
            red = 0
            for local_y in range(max(0, y - radius), min(image.shape[0], y + radius + 1)):
                for local_x in range(max(0, x - radius), min(image.shape[1], x + radius + 1)):
