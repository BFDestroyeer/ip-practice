...
def yuv_to_bgr(image):
    ...
    for y in range(image.shape[0]):
        for x in range(image.shape[1]):
            result_image[y, x, 0] = min(image[y, x, 0] + 2.032 * (image[y, x, 1] - 128), 255)
            result_image[y, x, 1] = min(image[y, x, 0] - 0.395 * (image[y, x, 1] - 128) - 0.581 * (image[y, x, 2] - 128), 255)
            result_image[y, x, 2] = min(image[y, x, 0] + 1.140 * (image[y, x, 2] - 128), 255)
    return result_image
...
def increase_brightness_yuv(image, coefficient):
    ...
    for y in range(image.shape[0]):
        for x in range(image.shape[1]):
            result_image[y, x, 0] = min(image[y, x, 0] * coefficient, 255)
            result_image[y, x, 1] = image[y, x, 1]
            result_image[y, x, 2] = image[y, x, 2]
    return result_image
...
def main(args):
    ...
    # Конвертация в YUV в форме BT.601 YCbCr
    manual_image = bgr_to_yuv(image)
    cv2.imwrite("output/yuv_manual.png", manual_image, )

    # Сравнение результатов конвертации
    print('YUV conversion MSE = ', comparison.compare_color_fast(cv2_image, manual_image))

    # Повышение яроксти
    brighter_bgr_start = time.time()
    brighter_image_bgr = increase_brightness_bgr(image, 1.2)
    brighter_bgr_end = time.time()

    brighter_yuv_start = time.time()
    brighter_image_yuv = increase_brightness_yuv(manual_image, 1.2)
    brighter_image_yuv = yuv_to_bgr(brighter_image_yuv)
    brighter_yuv_end = time.time()

    cv2.imwrite("output/brighter_bgr.png", brighter_image_bgr)
    cv2.imwrite("output/brighter_yuv.png", brighter_image_yuv)

    print("BGR Brightness increase filter time: ", brighter_bgr_end - brighter_bgr_start)
    print("YUV Brightness increase filter time: ", brighter_yuv_end - brighter_yuv_start)
    print("Brightness increase MES = ", comparison.compare_color_fast(brighter_image_bgr, brighter_image_yuv))
